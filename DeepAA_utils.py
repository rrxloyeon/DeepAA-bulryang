...
def aug_op_nepes_list():  # 16 oeprations and their ranges
    l = [
        (Identity, 0., 1.0), # 0
        (ShearX, -0.3, 0.3),  # 1
        (ShearY, -0.3, 0.3),  # 2
...
def get_dataset(args):
    ...
    if args.dataset in ['cifar10', 'cifar100']:
        ...
        x_train_, y_train_, x_val, y_val, x_test, y_test, num_class = get_dmd_data(data_root='/home/esoc/datasets/DMD/train')# "/home/esoc/datasets/DMD/train")#'./dataset_test')

        np.random.seed(args.seed)
        pt_idx=np.random.choice(len(x_train_),args.pretrain_size, replace=False)


        x_train, y_train = [x_train_[i] for i in pt_idx], [y_train_[i] for i in pt_idx]
        x_search, y_search = np.delete(x_train_, pt_idx, 0), np.delete(y_train_, pt_idx, 0)
